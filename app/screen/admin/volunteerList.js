...
import {getVolunteerBoothDetails, getVolunteerChanges} from "../../redux/actions/volunteerAction";
import {ImageFullScreenPreview} from "../common/ImageFullScreenPreview";


const VolunteerListToChangeVoterDetails = props => {
    ...
    const displayDetailPage = index => {
        ...
        dispatch(getVolunteerChanges({volunteerId:tempData?.VoterId})).then((res)=>{
            if(res){
                props.navigation.navigate('VolunteerChanges',{volunteerId:tempData?.VoterId})
            }
        })
...
export default VolunteerListToChangeVoterDetails;
