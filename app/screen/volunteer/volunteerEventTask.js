...
const VolunteerEventTask = props => {
    ...
    const renderTaskList = ({item, index}) => {
        return (
                ...
                <View key={Math.random() + 'DE'} style={{flex: 1,marginTop:hp(2)}}>
                    <View style={{padding:hp(1),borderRadius:hp(1),alignSelf:'center',backgroundColor:item.Status!=='Pending'?color.lightGreen:'orange',flex:1,width:wp(95)}}>
                        ...
                        <View style={{flexDirection:'row'}}>
                            ...
                            <View style={{marginTop:hp(1),alignItems:'center',justifyContent:'center'}}>
                                <TouchableOpacity onPress={()=>{
                                    let obj = {
                                        taskId:item?.TaskId,
                                        workStatus:item?.Status === 'Pending'?'1':'0'??'0'
                                    }
                                    updateTaskAndToDb(obj)

                                }}>
                                <View style={{height:hp(3),width:wp(40),backgroundColor:color.themePurple,borderRadius:hp(2),alignItems:'center',justifyContent:'center'}}>
                                    <Text style={{fontWeight:'700',fontSize:normalize(12),color:color.white}}>UPDATE STATUS</Text>
                                </View>
                                </TouchableOpacity>
                            </View>
                    </View>
        ...
        Alert.alert(
            ...
            [
                ...
                },
                {
                    text: 'Yes',
                    onPress: async () => {
                        dispatch(updateTaskInformation(data)).then((res)=>{
                            if(res){
                                dispatch(fetchVolunteerTask()).then((res)=>{})
    ...
    const renderVolunteerList = ({item, index}) => {
                ...
                <View key={Math.random() + 'DE'} style={{flex: 1,marginTop:hp(2)}}>
                    <View style={{padding:hp(1),borderRadius:hp(1),alignSelf:'center',backgroundColor:item.VolunteerId!==null?color.lightGreen:'orange',flex:1,width:wp(95)}}>
                        <View style={{flexDirection:'row'}}>
                            ...
                            <Text style={styles.eventValue}>{item?.FirstName + " " + item?.LastName}</Text>
                        </View>
                        <View style={{flexDirection:'row'}}>
        ...
        return (
        <View style={{flex: 1}}>
            <GoBackHeader title={TaskInformation[TaskDataIndex]?.eventData?.EventName} onMenuPress={() => props.navigation.goBack()} />
            ...
            <FlatList
                ...
                data={TaskInformation[TaskDataIndex].eventTask.length>0 && TaskInformation[TaskDataIndex].eventTask}
...
export default VolunteerEventTask;
