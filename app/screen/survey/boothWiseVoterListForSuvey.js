...
import {getSurveyQuestionList} from "../../redux/actions/surveyAction";
const BoothWiseVoterListForSurvey = props => {
    const dispatch = useDispatch()
    const {boothName = '',boothId = 1,surveyId=1} = props.route.params
    let openRowRef = null;
    ...
    const voterList = useSelector(state => state.surveyReducer.voterList);
    ...
    useEffect(()=>{
        dispatch(getSurveyQuestionList({surveyId})).then(async (res)=>{
            if(res){
                await setSurveyQuestion(res)
            }
        })
    },[])
    const displayDetailPage = async index => {
        if(surveyQuestion.length>0 && typeof surveyQuestion!=='undefined'){
            let defaultObj = {};
            await surveyQuestion.map((item,index)=>{
                defaultObj = {...defaultObj,[item?.QuestionId]:''}
            })
            props.navigation.navigate('SuverQuestionListForVolunteer', {
                data: surveyQuestion,
                surveyId,
                defaultObj,
                boothId,
                voterId:voterList[index].VoterId
            });
        } else {
                        ...
                        <View style={{flex: 1, justifyContent: 'space-between'}}>
                            ...
                            <View style={{flexDirection:'row',flex:1,justifyContent: 'space-between'}}>
                                ...
                                <View style={{alignItems:'center',justifyContent:'center',height:hp(2.5),width:wp(25),borderRadius:hp(1.5),backgroundColor:item?.TrustFactor?.Color??'red'}}>
                                    <Text style={{fontSize:normalize(8),fontWeight:'700'}}>{item?.TrustFactor?.Name??'AGAINST'}</Text>
                                </View>
                            </View>
...
export default BoothWiseVoterListForSurvey;
